...
path_mails =  localdir +'/static/data/processed'


path_corrections =  localdir + '/static/data/corrections/'
...
def openConnectionToPostgresDb():
    conn = psycopg2.connect("dbname=naml user=naml password=naml host=db")
    cur = conn.cursor()
    return conn, cur
...
def init_database():
    ...
    connSqlite3 = sqlite3.connect(path_database + filename_database )
...
def get_Image_names():
    ...
    print(os.getcwd())
    with open(localdir+'/static/Images/json_filenames_NA.txt', 'r') as outfile:
        filenames = json.load(outfile)
    return filenames

def get_Email_names(user='Mette'):
    with open(localdir+'/static/Images/Emails/Users/'+user+'/json_email_data_NA.txt', 'r') as outfile:
